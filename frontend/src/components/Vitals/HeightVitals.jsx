...
const HeightVitals = ({
  ...
  useEffect(() => {
    if (statusMessageh === "Reading Complete") {
      const timer = setTimeout(() => setStatusMessageH(""), 5000);
  ...
  const StatusMessage = ({ message }) => {
    ...
    switch (message) {
      case "Hardware Configured":
        ...
        Icon = FaCheckCircle;
        break;
      case "Height Sensor Initialized":
        bgColor = "bg-green-100 border-l-4 border-green-500";
        textColor = "text-green-800";
        Icon = FaCheckCircle;
        break;
      ...
      {!profileExists ? (
        ...
        <>
          <div className="mt-6">
            ...
            <button
              onClick={requestHeight}
              disabled={!connected || loadingh || !hardwareConfigured}
              className={`mt-2 py-2 px-6 rounded-md shadow-md transition duration-300 text-white ${
                !connected || loadingh || !hardwareConfigured
          ...
          {statusMessageh && <StatusMessage message={statusMessageh} />}
          ...
          {height && (
            <p className="mt-6 text-lg text-gray-800">
              Height:{" "}
              <span className="font-bold text-blue-600">{height} cm </span>
            </p>
          )}
        </>
      )}
